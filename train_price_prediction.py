...
def main():
    print("=" * 60)
    print("TASK 4: PRICE PREDICTION")
    print("=" * 60)

    # ---------------------------------------------------------
    # STEP 1 — EXPLORE
    # ---------------------------------------------------------
    data_dir = r"C:\Users\Dhanush\OneDrive\Desktop\CL\datasets\price_prediction"
    ...
    # Encode any categorical columns (e.g. state, district, market, crop if present)
    ...
    price_sub_dir = os.path.join(models_dir, "price_prediction")
    os.makedirs(price_sub_dir, exist_ok=True)

    model_path = os.path.join(models_dir, "price_prediction_model.pkl")
    encoders_path = os.path.join(models_dir, "price_encoders.pkl")
    feature_order_path = os.path.join(models_dir, "price_feature_order.json")

    joblib.dump(best_model, model_path)
    joblib.dump(best_model, os.path.join(price_sub_dir, "price_prediction_model.pkl"))
    joblib.dump(encoders, encoders_path)
    joblib.dump(encoders, os.path.join(price_sub_dir, "price_encoders.pkl"))
    ...
    with open(os.path.join(price_sub_dir, "feature_order.json"), "w") as f:
        json.dump(feature_order_data, f, indent=4)

    print(f"Saved model: {model_path}")
    print(f"Saved encoders: {encoders_path}")
    print(f"Saved feature order: {feature_order_path}")
    print("\nTask 4 Price Prediction completed successfully.")
